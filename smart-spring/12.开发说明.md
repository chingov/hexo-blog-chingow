...
```
* 实现IReturnCode接口，添加异常业务类型（code，message）
...
```
api包resources目录dubbo文件夹下dubbo-consume-demo.xml中
...
```
* 定义接口后需要在consume.xml中声明，validation为可选属性，缺省值是false，建议开启。<br>开启验证后注解`NotBlank`会生效，对接口参数进行校验。
...
```java
@POST
public String demo1(@BeanParam DemoFormVo demo) {
    return "hello word";
}
...
* 使用方式与Restful接口类似，不同的是注解不太一样（`SpringMVC的注解`）

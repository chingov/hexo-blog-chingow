...
* 在 *resources.META-INF.spring* 目录（缺省目录）内新建配置文件`dubbo-provider.xml`
    ...
    <beans xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
        ...
        <dubbo:registry address="${dubbo.registry.address}" check="false"   timeout="${dubbo.registry.timeout:30000}"/>
...
---
##### 12. 处理自定义异常
* 缺省处理异常class为 `com.iflytek.sgy.social.web.core.exception.ExceptionDispatcherServlet` ，在*web.xml* 里配置SpringMVC的` servlet-class`即可。
* 如想处理自定义异常（*注：此功能在 V 0.5.0版本后方可使用*），自定义实现类继承`com.iflytek.sgy.social.web.core.exception.AbstractExceptionDispatcherServlet`，重载`handleOtherException()`方法，配置` servlet-class`为实现类。
